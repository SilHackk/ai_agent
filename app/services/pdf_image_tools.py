...
def pdf_to_preview_images(pdf_path: str, output_dir: str = "uploads/pdf_previews", zoom: float = 0.55) -> List[Dict]:
    """
    PDF paverčia į mažus preview paveikslus GPT Vision atrankai.
    Čia nėra galutinė analizės kokybė – tik greitas puslapių atsirinkimas.
    """
...
def select_relevant_pdf_pages_with_gpt(
    ...
    """
    GPT Vision atrenka, kurie PDF puslapiai aktualūs langų/brėžinių analizei.
    ...
    for start in range(0, len(previews), batch_size):
        ...
        content = [{
            ...
            "text": (
                ...
                "nereikalingi architektūriniai puslapiai be langų/durų informacijos.\n\n"
                ...
                "    \"reason\": \"kodėl puslapis aktualus arba ne\",\n"
                "    \"expected_content\": [\"window drawing\", \"dimensions\"]\n"
                "  }\n"
                "]\n\n"
        ...
        for item in batch:
            content.append({
                "type": "input_text",
                "text": (
                    f"Puslapis {item['display_page']} / page_number {item['page_number']}.\n"
                    f"Teksto preview:\n{item['text_preview']}"
                )
            })
            content.append({
                "type": "input_image",
                "image_url": _image_to_data_url(item["image_path"])
            })
        ...
        try:
            ...
            # fallback: jei GPT grąžino blogą JSON, bent nepralaužiam sistemos
            continue

    selected_pages = sorted(
        selected_pages,
        key=lambda x: 0 if x.get("relevance") == "high" else 1
    )
...
def select_relevant_pdf_pages(pdf_path: str):
    """
    Šitą funkciją jau kviečia project_analyzer.py.
    Dabar ji naudoja GPT Vision, nebe pirmus 3 puslapius.
    """
    ...
    if not selected:
        return [{
            ...
            "display_page": 1,
            "relevance": "fallback",
            "reason": "GPT Vision nerado aiškiai aktualių puslapių, todėl paimtas pirmas puslapis.",
            "expected_content": []
        }]
...
def pdf_to_images(pdf_path: str, pages: List[int] | None = None, output_dir: str = "uploads/pdf_pages", zoom: float = 2.0) -> List[str]:
    """
    PDF puslapius paverčia į PNG paveikslus analizei.
    pages turi būti 0-based indeksai: pvz. [0, 2, 5]
    """
    os.makedirs(output_dir, exist_ok=True)
