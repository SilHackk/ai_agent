...
def create_project(source='manual', client_email=None, subject=None, email_text='') -> int:
    init_db()
    with get_conn() as conn:
        cur = conn.execute(
            'INSERT INTO projects(source, client_email, subject, email_text, created_at) VALUES (?, ?, ?, ?, ?)',
            (source, client_email, subject, email_text, now())
        )
